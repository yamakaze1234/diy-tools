-- Stage A/B only. No business baseline or real user accounts are seeded here.
CREATE TABLE public.wb_workspaces (
 id text PRIMARY KEY, name text NOT NULL, head_seq bigint NOT NULL DEFAULT 0 CHECK(head_seq>=0),
 created_at timestamptz NOT NULL DEFAULT now()
);
CREATE TABLE public.wb_members (
 uid text PRIMARY KEY, workspace_id text NOT NULL REFERENCES public.wb_workspaces(id),
 member_id text NOT NULL UNIQUE, display_name text NOT NULL, active boolean NOT NULL DEFAULT true
);
CREATE TABLE public.wb_records (
 workspace_id text NOT NULL REFERENCES public.wb_workspaces(id), entity_type text NOT NULL,
 entity_id text NOT NULL, version bigint NOT NULL CHECK(version>0), data jsonb NOT NULL,
 updated_by text NOT NULL, updated_at timestamptz NOT NULL DEFAULT now(),
 PRIMARY KEY(workspace_id,entity_type,entity_id), CHECK(entity_type IN ('configuration','component'))
);
CREATE TABLE public.wb_changes (
 workspace_id text NOT NULL REFERENCES public.wb_workspaces(id), seq bigint NOT NULL,
...
CREATE TABLE public.wb_mutations (
 workspace_id text NOT NULL REFERENCES public.wb_workspaces(id), mutation_id text NOT NULL,
...
ALTER TABLE public.wb_workspaces ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.wb_members ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.wb_records ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.wb_changes ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.wb_mutations ENABLE ROW LEVEL SECURITY;
-- Tables are intentionally inaccessible to browsers. The sole data-plane entrance is the checked RPC below.
REVOKE ALL ON public.wb_workspaces, public.wb_members, public.wb_records, public.wb_changes, public.wb_mutations FROM anon, authenticated;

CREATE FUNCTION public.wb_api(request jsonb) RETURNS jsonb
...
BEGIN
 ...
 IF kind='component' THEN
   IF jsonb_typeof(desired->'goodsId') IS DISTINCT FROM 'string' OR coalesce(desired->>'goodsId','')!~'^[0-9]{1,20}$' OR coalesce(desired->>'erpScopeId','')='' OR NOT desired?'taxCents'
   ...
   END IF;
 ELSE
   IF desired->>'id' IS DISTINCT FROM rid OR coalesce(desired->>'shopId','') NOT IN ('intel','gigabyte','jonsbo') OR jsonb_typeof(desired->'parts') IS DISTINCT FROM 'array'
