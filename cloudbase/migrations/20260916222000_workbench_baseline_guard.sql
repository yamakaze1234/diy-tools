...
BEGIN
 ...
 IF kind='workspace_meta' AND NOT EXISTS(SELECT 1 FROM public.wa_records WHERE workspace_id=m.workspace_id AND entity_type='workspace_meta' AND entity_id=rid) THEN
   IF jsonb_typeof(desired->'counts') IS DISTINCT FROM 'object' THEN RETURN jsonb_build_object('ok',false,'code','INCOMPLETE_BASELINE'); END IF;
   FOR k IN SELECT jsonb_object_keys(desired->'counts') LOOP
     IF coalesce(desired->'counts'->>k,'')!~'^[0-9]+$' OR (SELECT count(*) FROM public.wa_records WHERE workspace_id=m.workspace_id AND entity_type=k)<(desired->'counts'->>k)::bigint THEN
       RETURN jsonb_build_object('ok',false,'code','INCOMPLETE_BASELINE','message','初始数据尚未完整上传');
     END IF;
   END LOOP;
 END IF;
