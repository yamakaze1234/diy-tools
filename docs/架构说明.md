...
适用版本：Python 桌面版 0.3.25。[返回首页](../README.md) · [使用教程](使用手册.md) · [功能说明书](DIY配置工作台-产品说明书.html)
...
Python 桌面端是主要入口。`prototype/` 保存桌面端与轻量网页版共用的界面模块和业务规则，Python 构建复用其前端与纯函数。**Python 成品运行时不启动 Node 或 Electron**；Node/esbuild 只用于开发构建，QuickJS 在 Python 内部执行有限的纯函数。`web-lite/` 通过浏览器和 IndexedDB 提供轻量入口，并复用云端协议。
...
| [prototype/](../prototype/) | 共用前端、海报渲染、商品标准、库存检查和导入导出规则 |
| [web-lite/](../web-lite/) | 轻量网页版、IndexedDB 状态与云端协议适配 |
