...
适用版本：Python 桌面版 0.3.3。[返回首页](../README.md)
...
flowchart LR
  subgraph PC[Windows 本机]
    UI[pywebview / WebView2 界面]
    API[Python 本机 HTTP 服务]
    Rules[QuickJS 共享纯函数]
    Local[(SQLite / JSON / 图片)]
    Sync[后台同步线程]
    UI -->|127.0.0.1| API
    API --> Rules
    API --> Local
  ...
  ERP[(ERP SQL Server)]
  Auth[CloudBase 成员认证]
  Function[workbenchApi 云函数]
  PG[(CloudBase PostgreSQL)]
  Assets[CloudBase 图片存储]
  UI --> Auth
  API -->|只读查询与预览| ERP
  Sync -->|HTTPS 与成员令牌| Function
  Function -->|wa_api RPC| PG
  Sync --> Assets
```
