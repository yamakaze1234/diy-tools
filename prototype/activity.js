...
const labels={addons:'加购与选配',benefits:'福利点',parts:'配件',actualParts:'实际配置',price:'售价',priceCents:'售价',name:'配置名称',shortName:'简称',spu:'SPU',skuId:'SKU',posterImages:'添加的图片',caseImage:'机箱图',modules:'模块',theme:'主题',palette:'配色',tax:'核算价',taxCents:'核算价',upgrade:'升级文案',addonText:'加购文案'};
...
export function activityEntry(type,id,before,after,{at=new Date().toISOString(),operator='未知成员',origin='local',eventId,message}={}){
 ...
 if(type==='configuration'){
  ...
  target.tab=fields.includes('actualParts')?'actual':fields.includes('addons')||fields.includes('benefits')?'addons':fields.includes('parts')?'parts':fields.some(k=>['price','priceCents'].includes(k))?'costs':target.field?'parts':'modules';
