...
export function bindChangeHistory({api,flush,openDialog,esc,toast}){
 ...
 async function render(){
  ...
  const tabs=`<div class="actions"><button id="changes-local" ${active==='local-history'?'class="primary"':''}>本机修改记录</button><button id="changes-compare" ${active==='compare'?'class="primary"':''}>本机 / 云端差异</button><button id="changes-cloud" ${active==='commits'?'class="primary"':''}>云端提交记录</button></div>`;
  ...
  if(active==='local-history'){
   ...
   description='已被云端接受并产生数据变更的提交；时间来自云端服务器。每条业务记录独立提交，一轮同步可能产生多条提交。其他成员暂显示成员 ID。查看不会执行同步。';
  ...
  openDialog('修改与提交记录',`${tabs}<p>${description}</p><p id="change-message" class="error" role="alert"></p>${content||'<p>暂无记录或差异。</p>'}<div class="actions"><button id="changes-refresh">刷新 / 返回最新</button>${result.nextBefore?'<button id="changes-older">更早记录</button>':''}</div>`);
