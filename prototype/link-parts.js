...
export function openLinkParts(api){
 ...
 $('#link-part-query').oninput=()=>{invalidate();$('#link-part-match').innerHTML='<option value="">请选择原配件</option>'+(Object.values(scope()).some(Boolean)?scopedPartUsage(configs(),$('#link-part-query').value,scope()):[]).map(r=>`<option value="${esc(r.key)}">${esc(r.name)} · ${r.count} 套配置</option>`).join('');};
 for(const id of ['link-part-display','link-part-actual'])$('#'+id).onchange=()=>{invalidate();$('#link-part-query').oninput();$('#link-part-status').textContent=Object.values(scope()).some(Boolean)?'修改内容已变化，请重新选择原配件并预览':'请至少勾选展示配件或实际配置';};
 ...
 $('#link-part-preview').onclick=()=>{try{
  ...
  plan=linkPartsPlan(targets,{scope:scope(),mode:add?'add':'replace',partKey:$('#link-part-match').value,row:picker.row(),slot:$('#link-part-slot').value,qty:qty===''?null:Number(qty)});
