...
export function posterImageRegion(layer,layout,base,height,transform=layer.transforms?.[layout]||{}){
 ...
 const maxWidth=Math.min(base,height/ratio),minWidth=Math.min(16,maxWidth);
 ...
 return{key:'poster-image:'+layer.id,id:layer.id,x:clamp(Number.isFinite(transform.x)?transform.x:(base-width)/2,0,base-width),top:clamp(Number.isFinite(transform.y)?transform.y:(height-imageHeight)/2,0,height-imageHeight),width,height:imageHeight,minWidth,maxWidth};
}
