...
export const shops=[
 ...
 {id:SHOP_IDS.jonsbo,name:'乔思伯官方旗舰店',shortName:'乔思伯',brandText:'JONSBO',defaultTheme:'dark',brandLogo:{src:'/assets/jonsbo-logo-black.png',crop:[59,96,1921,643]}}
];
...
export function shopPalette(shopId,theme='light'){
 if(shopId===SHOP_IDS.gigabyte)return theme==='dark'?{bg:'#101010',text:'#ffffff',accent:'#ff7900',line:'#353535',muted:'#b8b8b8',panel:'#1d1d1d'}:{bg:'#ffffff',text:'#171717',accent:'#f36c21',line:'#dddddd',muted:'#747474',panel:'#f3f3f3'};
...
export const shopServiceColor=(shopId,theme)=>shopId===SHOP_IDS.jonsbo?(theme==='dark'?'#2c4353':'#3b5870'):shopId===SHOP_IDS.gigabyte?'#f36c21':'#173e76';

export const shopAddonColors=(shopId,theme)=>shopId===SHOP_IDS.jonsbo?(theme==='dark'?{bg:'#dbc3a0',text:'#342c24',muted:'#61503e',title:'#56412b'}:{bg:'#29485c',text:'#ffffff',muted:'#d0dce5',title:'#f2d3a1'}):null;
...
export function switchShopTheme(config,theme){
 ...
 for(const module of config.modules||[])if(module.type==='service')module.background=shopServiceColor(config.shopId,theme);
 return config;
}
...
export function applyShopIdentity(config,shopId,{resetAppearance=false}={}){
 ...
 if(resetAppearance){
  ...
  for(const module of config.modules||[]){module.color='';if(module.type==='service')module.background=shopServiceColor(shop.id,config.theme);}
 }
 return config;
}
...
function normalizeShopSettings(value){
 ...
 const result={};for(const shop of shops){const entry=value?.[shop.id];result[shop.id]={coupon:Number.isFinite(Number(entry?.coupon))&&Number(entry.coupon)>=0?Number(entry.coupon):shop.id===SHOP_IDS.intel&&Number.isFinite(legacyCoupon)&&legacyCoupon>=0?legacyCoupon:0};}
 return result;
}
...
export function normalizeWorkspaceState(input,baseCatalog=[]){
 ...
 state.sourceCatalog=normalizeSources(state.sourceCatalog?.length?state.sourceCatalog:baseCatalog);
...
export const sourcesFor=(state,shopId)=>preferredSourceRows(selectableComponents(state.sourceCatalog,state.costSource||[]).filter(row=>row.shopId===shopId),state.costSource||[]);
