...
export function openSourceEditor(liveApi){
 ...
 api.openDialog('本店输出源维护',`<div class="source-batch-bar"><strong id="source-pending">暂无待保存修改</strong><button id="source-save-all" class="primary">保存全部更改</button><button id="source-discard-all">放弃全部更改</button><p class="hint">切换商品会保留修改；全部编辑完成后统一保存到本机。</p></div><section id="source-sync-top" class="source-sync-top"></section><p class="hint">显示名称、升级文案属于当前店铺；加购集中展示三店，勾选要修改的店铺；含税核算单价按 ERP ID 与三店共用。核算价格为单件价，加购文案不计入整机成本。</p><div class="source-toolbar"><input id="source-search" type="search" placeholder="搜索显示名称、ERP ID 或升级文案" aria-label="搜索输出源"><button id="source-export" title="导出本店全部在用输出源，不受搜索或分页限制，包含当前未保存内容">导出本店表格</button><button id="source-addon-check">一键检查加购差价</button><button id="source-deleted">已删除</button><button id="source-special-list">特殊配件</button><button id="source-new">＋ 新增配件</button></div><div id="source-addon-report"></div><div class="source-workspace"><div class="source-list-panel"><div id="source-count" class="hint"></div><div id="source-list"></div><div class="source-pagination"><button id="source-prev">上一页</button><span id="source-page"></span><button id="source-next">下一页</button></div></div><div id="source-detail"><p class="hint">从左侧选择需要维护的配件。</p></div></div><div id="source-message" role="status" class="hint"></div>`);
 ...
 const filtered=()=>{const q=$('#source-search').value.trim().toLowerCase();return api.getRows().filter(r=>!!r.deletedAt===showDeleted&&(!specialOnly||isSpecialComponent(r))&&[r.name,r.goodsId,r.upgrade].join(' ').toLowerCase().includes(q));};
 ...
 $('#source-export').onclick=()=>{
  ...
  try{
   ...
   if(draft&&!showDeleted){const row={...clone(draft),shopId:api.shopId(),goodsId:$('#source-id')?.value.trim()??draft.goodsId,name:$('#source-name')?.value??draft.name,upgrade:$('#source-upgrade')?.value??draft.upgrade,specialComponent:$('#source-special')?.checked??draft.specialComponent};const index=rows.findIndex(r=>r.sourceId===row.sourceId);if(index>=0)rows[index]=row;else if(row.name?.trim()||row.goodsId?.trim())rows.push(row);}
   ...
   message(`已导出本店全部 ${values.length} 条在用输出源，列顺序：goodsid（ERP ID）、ERP名称、展示名称、升级项。${pending.size||draft&&isEdited()?'包含当前未保存内容，导出不会自动保存更改。':''}`);
