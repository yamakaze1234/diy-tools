...
export function sqlStockPlan(state,rows,at=new Date().toISOString()){
 ...
 next.sharedCostScope=state.sharedCostScope||state.erpSync?.scope||'unbound';const manual=new Map();for(const r of [...(state.sourceCatalog||[]),...(state.configs||[]).flatMap(allConfigParts)])if(r.goodsId&&r.tax!=null){if(!manual.has(r.goodsId))manual.set(r.goodsId,new Set());manual.get(r.goodsId).add(r.tax);}
 ...
 const update=(record,area,configId)=>{if(!record.goodsId)return;const row=map.get(record.goodsId);if(!row){unmatched++;record.erpMissing=true;return;}matched++;const value=row.stockAvailable;if(record.stockAvailable!==value||record.erp!==row.erp)changes.push({erpBefore:record.erp??null,erpAfter:row.erp??null,area,configId,goodsId:record.goodsId,name:record.name||row.name,before:record.stockAvailable??null,after:value});record.stockAvailable=value;record.stockUpdatedAt=at;record.stockSource='company-sql';record.erp=row.erp??null;record.erpMissing=false;record.erpUnknown=row.erp==null;record.erpUpdatedAt=at;if(configId)configIds.add(configId);};
 for(const r of next.sourceCatalog||[])update(r,'输出源');for(const r of next.costSource||[])update(r,'成本源');for(const c of next.configs||[])if(!c.deletedAt)for(const p of allConfigParts(c))update(p,'配置',c.id);
