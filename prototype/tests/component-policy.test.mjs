...
test('每次完整 ERP 导入排除星号及任意后缀，店铺别名不能绕过，已有配置保留',()=>{
 ...
 let result=mergeErp(state,valid,[],'now');assert.equal(result.excluded,5);assert.deepEqual(result.next.costSource.map(r=>r.goodsId),['3']);assert.equal(sourcesFor(result.next,'intel').length,0);
 const synced=applyWorkspace(result.next,projectWorkspace(result.next));assert.equal(synced.configs[0].parts[0].tax,80);assert.equal(synced.configs[0].parts[0].erp,100);assert.equal(sourcesFor(synced,'intel').length,0);assert.equal(result.next.configs[0].parts[0].name,'已有配置配件');assert.equal(result.next.configs[0].parts[0].qty,1);assert.equal(state.costSource[0].name,'旧名称');
 ...
 result=mergeErp(result.next,[row('1','恢复配件'),row('3','正常配件')],[],'again');assert.equal(sourcesFor(result.next,'intel').length,1);assert.ok(result.next.costSource.some(r=>r.goodsId==='1'));
});
test('历史数据按 ERP 原名排除，普通感叹号不排除',()=>{
 const state=normalizeWorkspaceState({configs:[],sourceCatalog:[{sourceId:'s',shopId:'intel',goodsId:'1',name:'店铺别名'}],costSource:[row('1','ERP*!停用')]});assert.equal(sourcesFor(state,'intel').length,0);
