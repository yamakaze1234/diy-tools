...
test('合成样例成本，单价乘数量及利润',()=>{const t=totals(seed.configs[0]);assert.equal(t.erp,6520);assert.equal(t.tax,6440);assert.equal(t.basis,6807.96);assert.equal(t.taxProfit,-108.96);assert.equal(t.erpProfit,45.02);});
...
test('按到手价扣点利润公式随到手价和配件数量计算',()=>{const c=structuredClone(seed.configs[0]);c.price=7999;assert.equal(totals(c).taxProfit,1139.04);assert.equal(totals(c).erpProfit,1319.02);c.parts[0].qty+=1;assert.equal(totals(c).taxProfit,-260.96);assert.equal(totals(c).erpProfit,-80.98);});
