...
test('按含税成本加点，利润随到手价和配件数量计算',()=>{const c=structuredClone(seed.configs[0]);c.price=7999;assert.equal(totals(c).taxProfit,1201.4);assert.equal(totals(c).erpProfit,1319.02);c.parts[0].qty+=1;assert.equal(totals(c).taxProfit,-254.6);assert.equal(totals(c).erpProfit,-80.98);});
