...
test('selected links and retained descriptions exclude exact fields without touching other links or shops',()=>{
 ...
 applySourceSync(plan,configs,[row]);assert.equal(configs[0].parts[0].name,'原名称');assert.equal(configs[0].addons[0].text,'原加购');assert.equal(configs[0].parts[0].upgrade,'新加购');assert.equal(configs[1].parts[0].upgrade,'原升级');assert.equal(configs[2].parts[0].upgrade,'原升级');
...
test('copy transfers binding quantity descriptions and addons but reads current costs and is deep copied',()=>{
 ...
 pastePartPayload(target,0,payload,[row]);assert.equal(target.parts[0].goodsId,'101');assert.equal(target.parts[0].name,'新名称');assert.equal(target.parts[0].qty,2);assert.equal(target.parts[0].erp,100);assert.equal(target.parts[0].posterVisible,false);assert.equal(target.addons[0].text,'原加购');
