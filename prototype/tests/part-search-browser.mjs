...
try{
 ...
 await page.evaluate(async code=>{
  ...
  const currentCatalog=()=>[{sourceId:'b',shopId:'intel',goodsId:'200',name:'技嘉 B760M AORUS ELITE WIFI6E',tax:500,addonText:'+99元升级示例'},{sourceId:'c',shopId:'intel',goodsId:'201',name:'技嘉 B850M AORUS ELITE',tax:800},{sourceId:'gone',goodsId:'300',name:'已删除',deletedAt:'yes'}];
 ...
 const displayName=()=>page.locator('[data-display-name]');assert.equal(await displayName().count(),1);
 await displayName().fill('配置图专用 CPU 名称');await displayName().press('Tab');
 assert.equal(await page.evaluate(()=>c.parts[0].name),'英特尔 酷睿 i5 14600KF');
 assert.equal(await page.evaluate(()=>c.parts[0].displayName),'配置图专用 CPU 名称');
 assert.equal(await page.evaluate(()=>c.actualParts[0].name),'英特尔 酷睿 i5 14600KF');
 assert.equal(await page.evaluate(()=>c.parts[0].goodsId),'100');
 assert.equal(await page.evaluate(()=>edits),1);
 const input=()=>page.locator('[data-part-name]');await input().fill('技嘉');assert.equal(await page.locator('.product-result').count(),2);assert.equal(await page.evaluate(()=>edits),1);assert.equal(await page.evaluate(()=>c.parts[0].goodsId),'100');assert.equal(await page.locator('.product-results').evaluate(e=>getComputedStyle(e).position),'absolute');
 ...
 await input().fill('200');await page.locator('[data-product-id=b]').click();assert.equal(await input().inputValue(),'技嘉 B760M AORUS ELITE WIFI6E');assert.equal(await page.evaluate(()=>c.actualParts[0].goodsId),'200');assert.equal(await page.evaluate(()=>c.parts[0].qty),2);assert.equal(await page.evaluate(()=>c.parts[0].upgrade),'+99元升级示例');assert.equal(await page.evaluate(()=>c.addons.length),0);
 await input().fill('不存在');assert.ok((await page.locator('.product-results').textContent()).includes('没有匹配'));await page.locator('#outside').click();assert.equal(await input().inputValue(),'技嘉 B760M AORUS ELITE WIFI6E');assert.equal(await page.evaluate(()=>edits),2);
