...
test('本地业务状态与草稿在一个 SQLite 事务内提交',()=>{const s=new SyncStore(':memory:',{protocolVersion:2,validate:validateWorkspaceRecord});assert.throws(()=>s.editMany(projectWorkspace(fixture()),()=>{throw Error('落盘失败');}));assert.equal(s.list().length,0);assert.equal(s.meta('workspaceState'),undefined);s.close();});
test('保存回执遇到继续输入，保留独立更新并检测同字段冲突',()=>{const base=fixture(),local=fixture(),remote=fixture();local.configs[0].name='正在输入';remote.configs[0].footer='他人备注';const result=mergeEditingState(base,local,remote);assert.equal(result.conflict,false);assert.equal(result.state.configs[0].name,'正在输入');assert.equal(result.state.configs[0].footer,'他人备注');remote.configs[0].name='别人名称';assert.equal(mergeEditingState(base,local,remote).conflict,true);});
