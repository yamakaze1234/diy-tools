...
export function applyWorkspace(template,records){
 ...
 const localRows=new Map();for(const r of [...(template.sourceCatalog||[]),...(template.configs||[]).flatMap(c=>c.parts||[]),...(template.costSource||[])])if(r.goodsId)localRows.set(r.goodsId,{...localRows.get(r.goodsId),...r});
 ...
 const restore=value=>{if(!value||typeof value!=='object')return;for(const child of Object.values(value))if(child&&typeof child==='object')restore(child);if(typeof value.goodsId==='string'&&('name' in value||'slot' in value)){const local=localRows.get(value.goodsId);for(const key of localErpFields){if(local&&key in local)value[key]=copy(local[key]);}value.erp=local?.erp??null;value.stockAvailable=local?.stockAvailable??null;}};
 ...
 const enrich=r=>{const c=costs.get(r.goodsId);if(!c)return;for(const k of derived)if(k in c)r[k]=copy(c[k]);};
 next.sourceCatalog.forEach(enrich);next.configs.forEach(c=>{if(!c.deletedAt)c.parts.forEach(enrich);});return next;
}
...
export function migrationSummary(state){const records=projectWorkspace(state),roundtrip=applyWorkspace(state,records);const issues=[];
 ...
 for(const c of state.configs||[])for(const p of c.parts){const shared=costs.get(p.goodsId);if(shared&&cents(p.tax)!==cents(shared.tax))issues.push({code:'COST_DIFF',configId:c.id,goodsId:p.goodsId,message:'配置缓存成本与共享成本不同'});}
