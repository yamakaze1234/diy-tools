...
export async function createWorkspaceSync({root,local,getState,setState,json,body}){
 ...
 async function materialize(){if(!store.get('workspace_meta','root'))return;
  await setState(current=>{const next=applyWorkspace(current,store.list());const received=recentActivity(store.meta('activity')||[]).filter(r=>r.operator!==member?.memberId);store.setMeta('activity',recentActivity(store.meta('activity')||[]));next.logs=recentActivity([...received,...(current.logs||[])]);if(equal(projectWorkspace(current),projectWorkspace(next))&&equal(current.logs,next.logs))return current;next.revision=current.revision+1;next.updatedAt=new Date().toISOString();store.setMeta('workspaceState',next);return next;});
 }
 ...
 async function persist(before,next,expected=before){if(!enabled())return next;if(!store.get('workspace_meta','root'))throw Error('首次云端数据仍在下载，请稍后编辑');const changes=changesBetween(expected,next);let merged;store.editMany(changes,()=>{merged=applyWorkspace(next,store.list());return merged;});return merged;}
