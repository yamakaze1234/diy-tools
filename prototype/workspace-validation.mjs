...
export function validateWorkspaceRecord(type,id,data){
 ...
 if(type==='configuration'){
  ...
  const lines=new Set();for(const p of data.parts){if(!p.lineId||lines.has(p.lineId)||typeof p.goodsId!=='string'||!Number.isSafeInteger(p.qty)||p.qty<0||p.goodsId&&p.qty<1)throw Error('配件行 ID 或数量无效');lines.add(p.lineId);}
