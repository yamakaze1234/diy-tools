...
$pythonApp = Get-ChildItem -LiteralPath $releaseRoot -Directory -Filter 'python-*' |
    ForEach-Object { Get-ChildItem -LiteralPath $_.FullName -Directory -Filter 'DIY配置工作台-Python-*' } |
    ForEach-Object { Get-ChildItem -LiteralPath $_.FullName -File -Filter 'DIY配置工作台-Python-*.exe' } |
